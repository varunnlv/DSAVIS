    ...
    await highlightLine("line-1");
      for (let i = 0; i < tempData.length; i++) {
        ...
        while (j > 0 && tempData[j - 1] > value) {
          ...
          tempData[j] = tempData[j - 1]; // Shift element to the right  
      
          await highlightLine("line-6");
          j--;           
        }
        await animateSwap(tempData[i], tempData[j], tempData); // Animate shift
        ...
        resetHighlight(j, j - 1); // Reset highlight after comparison
      }
      setData(tempData);
      generateBlocks(tempData);
      setIsSorting(false);
